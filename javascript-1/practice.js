...
let doubled = myNumbers.map((element, arr, index) => {
  return element * 2;
});
...
let total = myNumbers.reduce((element, acc) => {
  return acc + element;
}, 0);
...
let myNumbersIndex = [];
myNumbers.forEach((index, element) => {
...
let enemies = people.filter((element, index, arr) => {
...
let totallyAwesome = people.reduce((acc, element, index) => {
